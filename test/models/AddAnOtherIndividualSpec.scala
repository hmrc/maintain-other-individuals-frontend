...
import models.AddAnOtherIndividual.{NoComplete, YesNow}
...
class AddAnOtherIndividualSpec extends AnyWordSpec with Matchers with ScalaCheckPropertyChecks with OptionValues {

  "AddAnOtherIndividual" must {

    "only include yes and no as values" in {

      AddAnOtherIndividual.values mustBe Seq(YesNow, NoComplete)
