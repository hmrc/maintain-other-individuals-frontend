...
class OutOfBoundsPageNotFoundViewSpec extends ViewBehaviours {
  ...
  "OutOfBoundsPageNotFound view" must {
    ...
    "display the bullet point links with the correct text and hrefs" in {
      ...
      val addOtherIndividualLink = links.get(1)

      addOtherIndividualLink.text()       mustBe messages(s"$messageKeyPrefix.bullet2")
      addOtherIndividualLink.attr("href") mustBe
        controllers.routes.AddAnOtherIndividualController.onPageLoad().url
    }
  }
