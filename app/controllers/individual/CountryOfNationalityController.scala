...
import pages.individual.CountryOfNationalityPage
...
import views.html.individual.CountryOfNationalityView
...
class CountryOfNationalityController @Inject() (
  ...
  private val form: Form[String] = formProvider.withPrefix("otherIndividual.countryOfNationality")
    ...
    standardActionSets.verifiedForUtr.andThen(nameAction).async { implicit request =>
      form
        ...
        .fold(
          formWithErrors =>
            Future
              .successful(BadRequest(view(formWithErrors, mode, request.otherIndividual, countryOptions.options()))),
          value =>
            for {
