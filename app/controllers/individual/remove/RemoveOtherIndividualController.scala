      ...
      .fold(
        ...
        value =>
          if (value) {
            ...
            trustService.getOtherIndividual(request.userAnswers.identifier, index).flatMap { otherIndividual =>
              if (otherIndividual.provisional) {
                trustService
                  .removeOtherIndividual(request.userAnswers.identifier, RemoveOtherIndividual(index))
