...
import models.{
  Address, CombinedPassportOrIdCard, IdCard, NationalInsuranceNumber, NonUkAddress, OtherIndividual, Passport,
  UkAddress, UserAnswers, YesNoDontKnow
}
...
class OtherIndividualExtractor @Inject() () {
  ...
  private def extractIdentification(individual: OtherIndividual, answers: UserAnswers): Try[UserAnswers] =
    if (answers.isTaxable) {
